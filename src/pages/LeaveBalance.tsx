...
import { employees, leaveBalances, departments } from '@/data/sampleData';
...
export default function LeaveBalance() {
  ...
  const filteredData = useMemo(() => {
    return employees
      ...
      .map((emp) => ({
        employee: emp,
        balance: leaveBalances.find((lb) => lb.employeeId === emp.id),
      }));
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-xl border border-border bg-card hr-shadow-card overflow-hidden animate-slide-up">
        <Table>
          ...
          <TableBody>
            {filteredData.map(({ employee, balance }) => {
              const initials = `${employee.firstName[0]}${employee.lastName[0]}`;
              const totalAvailable =
                (balance?.casual.balance || 0) +
                (balance?.sick.balance || 0) +
                (balance?.earned.balance || 0);
                ...
                <TableRow key={employee.id}>
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground">
                          {balance?.casual.balance || 0} / {balance?.casual.total || 12}
                        </span>
                      </div>
                      <Progress
                        value={
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground">
                          {balance?.sick.balance || 0} / {balance?.sick.total || 10}
                        </span>
                      </div>
                      <Progress
                        value={
                          ((balance?.sick.balance || 0) /
                            (balance?.sick.total || 10)) *
                          100
                        }
                        className="h-2"
                      />
                  ...
                  <TableCell>
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-sm">
                        <span className="text-muted-foreground">
                          {balance?.earned.balance || 0} / {balance?.earned.total || 15}
                        </span>
                      </div>
                      <Progress
                        value={
                          ((balance?.earned.balance || 0) /
                            (balance?.earned.total || 15)) *
                          100
                        }
                        className="h-2"
                      />
