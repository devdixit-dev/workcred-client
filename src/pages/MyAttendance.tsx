...
import { attendance as allAttendance } from '@/data/sampleData';
          ...
          <div className="flex flex-col sm:flex-row items-center gap-6">
            <div className="flex items-center gap-4">
              ...
              <div className="text-center">
                ...
                <p className="text-2xl font-bold text-foreground">{todayAttendance?.checkIn || '--:--'}</p>
              </div>
              <div className="h-12 w-px bg-border" />
              <div className="text-center">
                <p className="text-sm text-muted-foreground mb-1">Check Out</p>
                <p className="text-2xl font-bold text-foreground">{todayAttendance?.checkOut || '--:--'}</p>
              </div>
              <div className="h-12 w-px bg-border" />
              <div className="text-center">
                <p className="text-sm text-muted-foreground mb-1">Working Hours</p>
                <p className="text-2xl font-bold text-foreground">
                  {company?.workingHours ? `${company.workingHours.start} - ${company.workingHours.end}` : '09:00 - 18:00'}
                </p>
              </div>
            </div>
          </div>
            ...
            {daysInMonth.map((day) => {
              ...
              return (
                <div
                  key={day.toISOString()}
                  className={cn(
                    'relative p-2 rounded-lg text-center min-h-[60px] flex flex-col items-center justify-center',
                    isToday(day) && 'ring-2 ring-primary',
                    attendance ? getStatusColor(attendance.status) : isWeekend ? 'bg-secondary/50' : 'bg-secondary/20'
                  )}
                >
                  <span className="text-sm font-medium">{format(day, 'd')}</span>
                  {attendance && attendance.checkIn && (
                    <span className="text-xs opacity-80">{attendance.checkIn}</span>
                  )}
                </div>
              );
            })}
